...
function buildSyncPacket(player){
    if(player.visible_entities === 0) return;
    bufferWriter.idx = 0;
    ...
    for(i = 0 ; i < players.length; i++){
        ...
        if(entered_viewport.length > 0){
            ...
            for( e = 0; e < entered_viewport.length; e++){
                ...
                entity = entered_viewport[e];

                bufferWriter.packInt8(entity.type);
                bufferWriter.packInt16(entity.id);
                bufferWriter.packInt16(entity.x);
                bufferWriter.packInt16(entity.y);
                bufferWriter.packInt8(entity.angle);
                ...
                switch(entity.type){
                  case 0:
                    ...
                    break;
                  case 1: //bullets
                    bufferWriter.packInt8(entity.speed);
                    break;
                  default:
...
var gameLoop = function () {
  ...
  if (previousTick + tickLengthMs <= now) {
    ...
    const tickLength = hrtimeMs() - then;
    console.log(`Tick took: ${tickLength}`);
